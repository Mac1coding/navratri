...
$("#close-btn").click(function(){
  $(".popup").hide();
  $("#cert-img").attr("src","./assets/cert.jpeg");
})
...
const capitalizeLetter = s => s.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');

function loadImage(url) {
  return new Promise(r => { let i = new Image(); i.onload = (() => r(i)); i.src = url; });
}

async function drawImage(name){
  name = capitalizeLetter(name);

  let image = await loadImage("./assets/cert.jpeg"); // Create new img element
  ...
  ctx.fillStyle = "#e5b666";
  ctx.textAlign = "center";
  ctx.font = "50px Arial";
  ctx.fillText(name, width/2, 412);

  let dataURL = canvas.toDataURL();
  $("#cert-img").attr("src",dataURL);
...
function CountdownTracker(label, value){
  ...
  el.innerHTML = '<b class="flip-clock__card card-clock"><b class="card__top"></b><b class="card__bottom"></b><b class="card__back"><b class="card__bottom"></b></b></b>' +
